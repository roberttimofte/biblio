		...
		<div class="container">
			...
			<form>
					...
					<div class="col-sm-8">
						<select class="form-control" id="editrice" name="editrice">
							...
							<?php
								...
								$sql = "SELECT EDITRICECont, EDITRICERagSoc FROM biblio_editrice";
								$result = mysqli_query($connessione, $sql);
								
								if (mysqli_num_rows($result) > 0) {
									while($row = mysqli_fetch_assoc($result)) {
										echo "<option value=". $row["EDITRICECont"] .">" . $row["EDITRICERagSoc"] . "</option>";
									}
								} else {
				...
				<div class="form-group row">
					...
					<div class="col-sm-2">
						<input type="text" class="form-control" placeholder="n pag" name="num_pagine">
					</div>
				</div>
